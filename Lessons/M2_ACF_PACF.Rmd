...
ggplot(inflow_data, aes(x=my_date, y=HP1)) + geom_line()
...
#only works if there is one source of seasonality. annual = 1, daily = 365. If not seasonal, ignore frequency
ts_inflow_data 
...
```{r acf}
#you want the lags to decay with time
# Option 1: The function acf() and pacf() is from package "stats"
HP1_acf=acf(ts_inflow_data[,1], lag.max = 40, plot=TRUE)
# blue dashes are the confidence intervals. lags that fall within the blue lines can be considered insignificant
#ts_inflow_data[,1], lag 1
#ts_inflow_data[,3], lag 3 (column)
...
HP1_acf=Acf(ts_inflow_data[,3], lag.max = 60, plot=TRUE) #from the forecast package
#omits lag 0

HP1_pacf=Pacf(ts_inflow_data[,3], lag.max = 40, plot=TRUE)

#influence of lag 1 was so strong that something was positive instead of negative 
