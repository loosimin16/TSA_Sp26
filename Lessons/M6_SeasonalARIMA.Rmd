...
The seasonal part is slowly decaying, it is ...
...
```{r}
y <- ts_electricity_price[,"Residential"]

# A few common starting candidates after (d=1, D=1, s=12)
fit_011_011 <- Arima(y, order=c(0,1,1), seasonal=c(0,1,1), include.drift=FALSE)
fit_111_011 <- Arima(y, order=c(1,1,1), seasonal=c(0,1,1), include.drift=FALSE)
fit_012_011 <- Arima(y, order=c(0,1,2), seasonal=c(0,1,1), include.drift=FALSE)
fit_112_011 <- Arima(y, order=c(1,1,2), seasonal=c(0,1,1), include.drift=FALSE)
fit_011_111 <- Arima(y, order=c(0,1,1), seasonal=c(1,1,1), include.drift=FALSE)

# Compare models
AIC(fit_011_011, fit_111_011, fit_012_011, fit_112_011, fit_011_111)

# Choose the best-looking one (lowest AICc), then diagnose residuals
checkresiduals(fit_112_011)  # <- change this to your selected "best" fit

# Forecast from the chosen model
fc_manual <- forecast(fit_112_011, h=24)
autoplot(fc_manual) + ggtitle("Manual SARIMA Forecast (h=24)")
...
fit_auto_sarima
checkresiduals(fit_auto_sarima)

fc_auto_sarima <- forecast(fit_auto_sarima, h=24)
autoplot(fc_auto_sarima) + ggtitle("Auto SARIMA Forecast (h=24)")
...
```{r}
x <- deseasonal_residential_price

fit_auto_arima_deseason <- auto.arima(
  x,
  seasonal = FALSE,
  stepwise = FALSE,
  approximation = FALSE
...
fit_auto_arima_deseason
checkresiduals(fit_auto_arima_deseason)

fc_deseason <- forecast(fit_auto_arima_deseason, h=24)
autoplot(fc_deseason) + ggtitle("Auto ARIMA on Deseasonalized Series (h=24)")
