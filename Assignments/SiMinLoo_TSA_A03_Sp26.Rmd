...
Despite the statistical significance of the slope, the model explains very little of the overall variation in the data (R² ≈ 0.025).
...
```{r}
trend_hydro <- fitted(lm(Hydroelectric ~ Month, data = df))
trend_renew <- fitted(lm(Renewable ~ Month, data = df))

df$Hydro_detrended <- df$Hydroelectric - trend_hydro
df$Renew_detrended <- df$Renewable - trend_renew

hydro_det_ts <- ts(df$Hydro_detrended,
                   frequency = 12,
                   start = c(start_year, start_month))
renew_det_ts <- ts(df$Renew_detrended,
                   frequency = 12,
                   start = c(start_year, start_month))

plot(hydro_ts,
     main = "Hydroelectric Power (Original)",
     ylab = "Energy Units", xlab = "Time")

plot(hydro_det_ts,
     main = "Hydroelectric Power (Detrended)",
     ylab = "Detrended Units", xlab = "Time")

plot(renew_ts,
     main = "Total Renewable Energy (Original)",
     ylab = "Energy Units", xlab = "Time")

plot(renew_det_ts,
     main = "Total Renewable Energy (Detrended)",
     ylab = "Detrended Units", xlab = "Time")
...
For hydroelectric power consumption, the original series already fluctuates around a relatively stable level with no clear long-term trend. After detrending, the series appears almost unchanged aside from a slight recentering around zero.
...
```{r}
acf_renew_det <- autoplot(Acf(renew_det_ts, lag.max = 60, plot = FALSE)) +
  labs(title = "Renewable (Detrended) - ACF",
       x = "Lag", y = "ACF")

pacf_renew_det <- autoplot(Pacf(renew_det_ts, lag.max = 60, plot = FALSE)) +
  labs(title = "Renewable (Detrended) - PACF",
       x = "Lag", y = "PACF")

plot_grid(acf_renew_det, pacf_renew_det, nrow = 1)


acf_hydro_det <- autoplot(Acf(hydro_det_ts, lag.max = 60, plot = FALSE)) +
  labs(title = "Hydroelectric (Detrended) - ACF",
       x = "Lag", y = "ACF")

pacf_hydro_det <- autoplot(Pacf(hydro_det_ts, lag.max = 60, plot = FALSE)) +
  labs(title = "Hydroelectric (Detrended) - PACF",
       x = "Lag", y = "PACF")

plot_grid(acf_hydro_det, pacf_hydro_det, nrow = 1)
```
...
```{r}
df$MonthFactor <- factor(format(df$Month, "%m"),
                         levels = sprintf("%02d", 1:12),
                         labels = month.abb)

# Seasonal means models
lm_renew_seasonal <- lm(Renewable ~ MonthFactor, data = df)
lm_hydro_seasonal <- lm(Hydroelectric ~ MonthFactor, data = df)

# Print summaries
summary(lm_renew_seasonal)
summary(lm_hydro_seasonal)
```
The seasonal means regression for total renewable energy production shows that none of the monthly dummy variables are statistically significant at conventional levels. The overall model fit is extremely weak, with an R² of approximately 0.008. This indicates that average renewable energy production does not differ systematically across months. In other words, there is no evidence of a meaningful seasonal pattern in renewable energy production once month-to-month variation is accounted for.

In contrast, the seasonal means regression for hydroelectric power consumption reveals strong and statistically significant seasonal effects. Many monthly dummy coefficients are significant at the 1% or 0.1% level, with sizable magnitudes. For example, hydroelectric consumption is significantly higher in late spring and early summer (e.g., May and June) and significantly lower in late summer and fall (e.g., August through November) relative to the reference month. The model explains a substantial portion of the variation in the data, with an R² of approximately 0.47
...
```{r}
season_renew <- fitted(lm_renew_seasonal)
season_hydro <- fitted(lm_hydro_seasonal)

df$Renewable_deseason <- df$Renewable - season_renew
df$Hydro_deseason <- df$Hydroelectric - season_hydro

renew_deseason_ts <- ts(df$Renewable_deseason,
                        frequency = 12,
                        start = c(start_year, start_month))

hydro_deseason_ts <- ts(df$Hydro_deseason,
                        frequency = 12,
                        start = c(start_year, start_month))
...
plot(renew_ts,
     main = "Renewable Energy (Original)",
     ylab = "Energy Units", xlab = "Time")

plot(renew_deseason_ts,
     main = "Renewable Energy (Deseasonalized)",
     ylab = "Deseasonalized Units", xlab = "Time")

plot(hydro_ts,
     main = "Hydroelectric Power (Original)",
     ylab = "Energy Units", xlab = "Time")

plot(hydro_deseason_ts,
     main = "Hydroelectric Power (Deseasonalized)",
     ylab = "Deseasonalized Units", xlab = "Time")
...
acf_renew_des <- autoplot(Acf(renew_deseason_ts, lag.max = 60, plot = FALSE)) +
  labs(title = "Renewable (Deseasonalized) - ACF",
       x = "Lag", y = "ACF")

pacf_renew_des <- autoplot(Pacf(renew_deseason_ts, lag.max = 60, plot = FALSE)) +
  labs(title = "Renewable (Deseasonalized) - PACF",
       x = "Lag", y = "PACF")

plot_grid(acf_renew_des, pacf_renew_des, nrow = 1)

acf_hydro_des <- autoplot(Acf(hydro_deseason_ts, lag.max = 60, plot = FALSE)) +
  labs(title = "Hydroelectric (Deseasonalized) - ACF",
       x = "Lag", y = "ACF")

pacf_hydro_des <- autoplot(Pacf(hydro_deseason_ts, lag.max = 60, plot = FALSE)) +
  labs(title = "Hydroelectric (Deseasonalized) - PACF",
       x = "Lag", y = "PACF")

plot_grid(acf_hydro_des, pacf_hydro_des, nrow = 1)
```

Comparing the deseasonalized ACF and PACF with those from Q1, there is essentially no change in the correlation structure. The ACF remains very high at short lags and decays slowly across many lags, indicating strong persistence and non-stationarity. The PACF continues to show a dominant spike at lag 1, with most higher lags being insignificant.
